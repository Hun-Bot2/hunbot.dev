...
| N7 | Contract fixtures were 11,619 lines of committed near-duplicate | **FIXED 2026-09-16.** The eleven directories under `test/fixtures/validate-research-contract/` are generated at test time from the real contract (`test/helpers/contract-fixture.mjs`), each the current schema plus one stated mutation; the generator refuses a mutation that changes nothing. Removing them also exposed a real weakness: INV-09 **silently skipped** when `research-item-identity.md` was absent, so moving or renaming that document would have retired the check without a word. It is now required, and a generated fixture supplies it. 11,619 lines removed, 107 tests still pass. | Fixed |
