...
| N6 | **Non-deterministic build output** | **Added 2026-09-14, noticed by T09 while diffing renders.** The Library page's *Featured Topics* pill selection order varies between two builds of identical source. Pre-existing and unrelated to papers, but it now matters more: the topic collection went from 1 entry to 32, so the selection has real choice. It makes render-diffing — the technique that caught the `iclr` regression — unreliable for that page. Worth a deterministic sort; not pre-AWS work |
