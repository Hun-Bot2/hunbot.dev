...
1. **연구 질문 확정**
   한국어 모어 일본어 학습자의 발음·운율을 자동 평가한다.

2. **범위 제한**
   자유 대화보다 먼저 **문장 읽기 → 섀도잉 → 자유 대화** 순으로 진행한다.

3. **평가 항목 정의**
   음소 정확도, 장단음, 촉음, 모라 길이, 피치 악센트, 유창성, 원어민 유사도.

4. **선행 연구 조사**
   `Japanese pronunciation assessment`, `L1 Korean Japanese learners`, `prosody assessment`, `CAPT` 중심으로 조사한다.

5. **데이터 확보**
   일본어 원어민 음성과 한국인 학습자 음성을 찾고, 부족하면 직접 소규모 녹음한다.

6. **평가 라벨 설계**
   일본어 원어민 평가자가 각 음성에 1~5점과 오류 유형을 기록하게 한다.

7. **베이스라인 모델 비교**
   Whisper, XLS-R, wav2vec 2.0 계열을 그대로 사용해 성능을 측정한다.

8. **음성 특징 추출**
   ASR 결과뿐 아니라 F0, 음소 길이, 모라 길이, 말하기 속도, 휴지를 추출한다.

9. **LoRA 또는 Adapter 학습**
   가장 좋은 베이스 모델을 한국인 일본어 학습자 데이터로 추가 학습한다.

10. **성능 평가**
    모델 점수와 일본어 원어민 점수의 Pearson·Spearman 상관계수를 비교한다.

11. **실험 분석**
    기본 모델, 전체 미세조정, LoRA 모델을 비교하고 어떤 요소가 성능을 높였는지 확인한다.

12. **논문 작성**
    문제 정의 → 데이터 → 모델 → 실험 → 결과 → 한계 순으로 작성한다.

13. **앱 MVP 제작**
    문장 재생 → 사용자 녹음 → 발음·억양 피드백 기능부터 구현한다.

핵심은 **처음부터 자유 대화를 만들지 말고, 문장 읽기 평가 모델부터 완성하는 것**이다.
